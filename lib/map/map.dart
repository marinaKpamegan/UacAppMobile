...
import 'package:uac_campus/map/navigation.dart';
...
class MapState extends State<UacMap> {
  ...
  void initState() {
    ...
    if(widget.startNavigation == true){
      _origin = WayPoint(name: widget.selectedLocation.name, latitude: widget.selectedLocation.lat, longitude: widget.selectedLocation.lng);
      _destination = WayPoint(name: widget.targetLocation.name, latitude: widget.targetLocation.lat, longitude: widget.targetLocation.lng);
      _isVisible = true;
      markers.add(
        new Marker(
            point: LatLng(widget.selectedLocation.lat, widget.selectedLocation.lng),
            builder: (ctx) =>
                InkWell(
                  child: Container(
                      child: Image.asset(placesIcon[widget.selectedLocation.type], width: 150, height: 192,)
                  ),
                  onTap: () {
          ...
          children: <Widget>[
            ...
            SafeArea(
              child:  FlutterMap(
                mapController: _mapController,
                options: MapOptions(
                  center: LatLng(widget.selectedLocation.lat, widget.selectedLocation.lng),
                  minZoom: 10,
                  maxZoom: 18.5,
                  zoom: 18.0,
                ),
           ...
           /* Positioned(
            ...
            Positioned(
              ...
              child: FloatingActionButton(
                ...
                child: Icon(Icons.home, color: Color.fromRGBO(35, 154, 105, 1),),
              ) ,
            ),
  ...
  Future<dynamic> readFileAsync(String filePath) async {
    ...
    List<UacLocation> uacLocation = parsed.map<UacLocation>((json)=> new UacLocation.fromJson(json)).toList();
      ...
      for (int i = 0; i <= uacPlaces.length - 1; i++){
        markers.add(Marker(
            ...
            builder: (ctx) =>
                InkWell(
                  child: Container(
                      child: Image.asset(placesIcon[uacPlaces[i].type], width: 150, height: 192,)
                  ),
                  onTap: () {
